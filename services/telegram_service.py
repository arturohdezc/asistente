...
class TelegramService:
    ...
    async def _handle_add_command(self, text: str, user_id: int) -> str:
        ...
        try:
            ...
            analysis_result = await self.gemini_service.analyze_text(
                text=text.strip(), source=f"telegram_user_{user_id}"
            )
            ...
            if not created_tasks:
                task = await self.task_service.create_task(
                    title=text.strip(), source=f"telegram_user_{user_id}", priority=Priority.NORMAL
                )
                created_tasks.append(task)
            ...
            # Escape special characters that might cause Telegram parsing issues
            error_msg = self._escape_telegram_text(str(e))
    ...
    async def _handle_done_command(self, args: str, user_id: int) -> str:
        ...
        try:
            ...
            updated_task = await self.task_service.update_task(
                task_id, {"status": TaskStatus.DONE.value}
            )
    ...
    async def _handle_calendar_command(self, args: str, user_id: int) -> str:
        ...
        try:
            ...
            analysis_result = await self.gemini_service.analyze_calendar_event(
                text=args.strip(), source=f"telegram_user_{user_id}"
            )
            ...
            logger.error(
                "Failed to create calendar event", args=args, user_id=user_id, error=str(e)
            )
            # Escape special characters that might cause Telegram parsing issues
            error_msg = self._escape_telegram_text(str(e))
    ...
    def _escape_telegram_text(self, text: str) -> str:
        """Escape special characters that might cause Telegram parsing issues"""
        return text.replace("*", "\\*").replace("_", "\\_").replace("[", "\\[").replace("]", "\\]")

    async def send_daily_summary(
        self, chat_id: int, tasks_by_priority: Dict[str, List[Task]]
    ) -> bool:
        ...
        try:
            ...
            logger.error("Failed to send daily summary", chat_id=chat_id, error=str(e))
            return False

    async def send_task_notification(
        self, chat_id: int, task: Task, notification_type: str = "created"
    ) -> bool:
        ...
        try:
            ...
            logger.error(
                "Failed to send task notification", chat_id=chat_id, task_id=task.id, error=str(e)
            )
            return False
    ...
    async def send_meeting_context(self, context_text: str, chat_id: int) -> bool:
        ...
        try:
            ...
            logger.error("Failed to send meeting context", chat_id=chat_id, error=str(e))
            return False
